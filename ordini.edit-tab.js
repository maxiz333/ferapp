...
var _editOrdPrezzoFocusOrigByIdx={};

function _editOrdPrezzoComparableValue(v){
  var raw=String(v==null?'':v).trim();
  if(!raw) return '';
  var n=parsePriceIT(raw);
  if(isFinite(n) && n>=0){
...
function _editOrdPrezzoOnFocus(idx,val){
  _editOrdPrezzoFocusOrigByIdx[idx]=String(val==null?'':val);
}

function _editOrdPrezzoOnBlur(idx,val){
  var hadOrig=Object.prototype.hasOwnProperty.call(_editOrdPrezzoFocusOrigByIdx, idx);
  var orig=hadOrig?_editOrdPrezzoFocusOrigByIdx[idx]:(_editOrdItems&&_editOrdItems[idx]?_editOrdItems[idx].prezzoUnit:'');
  if(hadOrig) delete _editOrdPrezzoFocusOrigByIdx[idx];
  if(_editOrdPrezzoComparableValue(orig)===_editOrdPrezzoComparableValue(val)) return;
  _editOrdPrezzo(idx,val);
}
