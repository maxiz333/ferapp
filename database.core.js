...
/** Ritorna l'indice catalogo per una riga articolo (carrello/ordine), se trovata. */
function findRowIdxByItemRef(it){
  if(!it || typeof rows === 'undefined' || !rows || !rows.length) return -1;
  if(it.rowIdx != null && it.rowIdx !== ''){
    var ri = parseInt(it.rowIdx, 10);
    if(!isNaN(ri) && rows[ri]) return ri;
  }
  var codM = String(it.codM || '').trim();
  var codF = String(it.codF || '').trim();
  if(!codM && !codF) return -1;
  for(var i = 0; i < rows.length; i++){
    var r = rows[i];
    if(!r) continue;
    if(codM && String(r.codM || '').trim() === codM) return i;
    if(codF && String(r.codF || '').trim() === codF) return i;
  }
  return -1;
}

/** True se l'articolo è promo Giornalino [G] (campo diretto o riga catalogo collegata). */
function isPromoGItem(it){
  if(!it) return false;
  if(it.isPromo === true && String(it.promoTipo || '') === 'G') return true;
  var ri = findRowIdxByItemRef(it);
  if(ri < 0 || !rows[ri]) return false;
  return rows[ri].isPromo === true && String(rows[ri].promoTipo || '') === 'G';
}

/** Badge promo Giornalino [G] da affiancare ai prezzi in UI. */
function htmlPromoGBadge(){
  return '<span class="promo-g-badge" title="Promo giornalino">[G]</span>';
}
